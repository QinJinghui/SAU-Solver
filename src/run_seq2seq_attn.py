    ...
    for epoch in range(n_epochs):
        ...
        for idx in range(len(input_lengths)):
            loss = train_seq2seq(input_batches[idx], input_lengths[idx], output_batches[idx], output_lengths[idx],
                                 ...
                                 use_teacher_forcing=1, beam_size=1, var_nums=var_num_ids, beam_search=True)
        ...
        if epoch % 10 == 0 or epoch > n_epochs - 5:
            ...
            # torch.save(encoder.state_dict(), "models/seq2seq_atten_encoder")
            # torch.save(decoder.state_dict(), "models/seq2seq_atten_decoder")
            # if epoch == n_epochs - 1:
            #     best_acc_fold.append((equation_ac, value_ac, eval_total))
            torch.save(encoder.state_dict(), os.path.join(current_save_dir, "seq2seq_atten_encoder"))
            torch.save(decoder.state_dict(), os.path.join(current_save_dir, "seq2seq_atten_decoder"))
            if best_val_acc < value_ac:
                best_val_acc = value_ac
                current_best_val_acc = (equation_ac, value_ac, eval_total)
                torch.save(encoder.state_dict(), os.path.join(current_save_dir, "seq2seq_atten_encoder_best_val_acc"))
                torch.save(decoder.state_dict(), os.path.join(current_save_dir, "seq2seq_atten_decoder_best_val_acc"))
            if best_equ_acc < equation_ac:
                best_equ_acc = equation_ac
                torch.save(encoder.state_dict(), os.path.join(current_save_dir, "seq2seq_atten_encoder_best_equ_acc"))
                torch.save(decoder.state_dict(), os.path.join(current_save_dir, "seq2seq_atten_decoder_best_equ_acc"))
            if epoch == n_epochs - 1:
                best_acc_fold.append((equation_ac, value_ac, eval_total))
